...
class CampaignController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data['year']=date('Y');
         //working with image
          ...
          Image::make($photo)->resize(468,90)->save('file/campaign/'.$photoname);  //image intervention
        $data['image']='file/campaign/'.$photoname;   // public/files/brand/plus-point.jpg
    ...
    public function update(Request $request)
    {
        ...
        if ($request->image) {
              ...
              $photo=$request->image;
              $photoname=$slug.'.'.$photo->getClientOriginalExtension();
              Image::make($photo)->resize(468,90)->save('file/campaign/'.$photoname); 
              $data['image']='file/campaign/'.$photoname; 
    ...
    {
           $products=DB::table('products')->leftJoin('catagories','products.category_id','catagories.id')
                    ->leftJoin('subcatagories','products.subcategory_id','subcatagories.id')
                    ->leftJoin('brands','products.brand_id','brands.id')
                    ->select('products.*','catagories.Catagory_name','subcatagories.Subcatagory_name','brands.brand_name')
